...
class Customer(ABC):
    ...
    def modify_cust_info(self, customer_id,
        ...
        '''Abstract method to modify customer
        info. of customers master file.'''
...
class HotelSystem(Hotel, Reservation):
    ...
    def create_hotel(self, new_data_file):
        ...
        try:
            ...
            print(f"Import finished: {added_count} new hotels "
                  f"added to {self.master_file}.\n")
    ...
    def delete_hotel(self, selected_id):
        ...
        updated_list = [h for h in master_list
                        if str(h.get('hotel_id')) != str(selected_id)]

        # Error handling by comparing hotel entries
        # to check if a hotel has been deleted.
        if len(updated_list) == initial_count:
            print(f"Error: Hotel ID '{selected_id}' "
                  f"not found in {self.master_file}.\n")
            return
    ...
    def display_hotel_info(self, hotel_id="all"):
        ...
        if not master_list:
            print(f"\n The master file '{self.master_file}' "
                  f"does not have any registered hotels.\n")
            return

        # Determine which hotels to show based on
        # hotel ID or if all are requested.
        if str(hotel_id).lower() == "all":
            display_list = master_list
        else:
            # Filter by the specific hotel ID and store info. in display list.
            display_list = [h for h in master_list
        ...
        for hotel in display_list:
            ...
            print(f"Hotel ID: {h_id:<3} | Name: {name:<15} "
                  f"| Location: {loc:<15} | "
    ...
    def modify_hotel_info(self, hotel_id, new_name=None, new_rooms=None):
        '''Function to modify hotel name and number
        of rooms in json master file.'''
        ...
        try:
            ...
            for hotel in master_list:
                # Check/select the hotel to be modified
                # by it hotel ID from master file.
    ...
    def reserve_room(self, hotel_id):
        '''Function to reserve a room in a hotel
        by decrementing the count in json master file.'''
        ...
        for hotel in master_list:
            if str(hotel.get('hotel_id')) == str(hotel_id):
                ...
                # Error handling, display message when there
                # are not any available rooms
                else:
    ...
    def create_reservation(self, customer_id, hotel_id,
        ...
        if not any(str(c.get('customer_id')) == str(customer_id)
                   for c in customers):
            print(f"Error: Customer ID '{customer_id}' "
                  f"not found in {customer_file}.")
            return
        ...
        target_hotel = next((h for h in hotels
                             if str(h.get('hotel_id')) == str(hotel_id)), None)
    ...
    def cancel_reservation(self, reservation_id,
                           res_file="master_reservations.json"):
        '''Function to cancel a reservation and
        return the room to the hotel inventory.'''
        ...
        print(f"Success: Reservation {reservation_id} cancelled. "
              f"Room returned to inventory.\n")
...
class CustomerSystem(Customer):
    ...
    def create_customer(self, new_data_file):
        ...
        try:
            ...
            print(f"Import finished: {added_count} new customers "
                  f"added to {self.master_file}.\n")
    ...
    def delete_customer(self, selected_id):
        ...
        initial_count = len(master_list)
        # Filter to keep all customers except the
        # customer selected to be deleted.
        updated_list = [c for c in master_list
                        if str(c.get('customer_id')) != str(selected_id)]

        # Error handling by comparing customer entries
        # to check if a customer has been deleted.
        if len(updated_list) == initial_count:
            print(f"Error: Customer ID '{selected_id}' "
                  f"not found in {self.master_file}.\n")
            return
    ...
    def display_cust_info(self, customer_id="all"):
        ...
        # Determine which customers to show based
        # on customer ID or if all are requested.
        if str(customer_id).lower() == "all":
            display_list = master_list
        else:
            # Filter by the specific customer ID and
            # store info. in display list.
            display_list = [c for c in master_list
        ...
        for customer in display_list:
            ...
            print(f"Customer ID: {c_id:<3} | Name: {name:<15} "
                  f"| Age: {age:<15} | "
                  f"Email: {email:<5}\n")

        print(f"\nCustomers displayed: {len(display_list)}.\n")

    def modify_cust_info(self, customer_id, new_name=None,
                         new_email=None):
        '''Function to modify customers name and email
        in json master file.'''
        ...
        try:
            ...
            for customer in master_list:
                # Check/select the customer to be modified by
                # its customer ID from master file.
...
def handle_standard_commands(command, target, value,
                             system_hotel, system_customer):
    """Handles create, delete, and display commands for both systems."""
    if target == "hotel":
        if command == "create":
            system_hotel.create_hotel(value)
        elif command == "delete":
            system_hotel.delete_hotel(value)
        elif command == "display":
            system_hotel.display_hotel_info(value)
    elif target == "customer":
        if command == "create":
            system_customer.create_customer(value)
        elif command == "delete":
            system_customer.delete_customer(value)
        elif command == "display":
            system_customer.display_cust_info(value)


def handle_modify_command(target, system_hotel, system_customer):
    """Handles the complex logic for modifying hotels or customers."""
    if target == "hotel":
        if len(sys.argv) == 5:
            ...
            system_hotel.modify_hotel_info(sys.argv[3],
        ...
        if len(sys.argv) == 5:
            ...
            system_customer.modify_cust_info(sys.argv[3],
                                             sys.argv[4], sys.argv[5])


def main():
    """Main entry point for the Hotel and Customer management CLI."""
    if len(sys.argv) < 2:
        print("Usage: python file.py <command> <target> <args>")
        return
    ...
    match command:
        case "create" | "delete" | "display":
            if len(sys.argv) >= 4:
                handle_standard_commands(command, sys.argv[2], sys.argv[3],
                                         system_hotel, system_customer)
        case "modify":
            if len(sys.argv) >= 3:
                handle_modify_command(sys.argv[2], system_hotel,
                                      system_customer)
        case "reserve":
            if len(sys.argv) >= 3:
                system_hotel.reserve_room(sys.argv[2])
        case "book":
            if len(sys.argv) >= 4:
                system_hotel.create_reservation(sys.argv[2], sys.argv[3])
        case "cancel":
            if len(sys.argv) >= 3:
